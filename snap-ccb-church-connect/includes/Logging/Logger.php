<?php
namespace SnapChurchConnect\CCB\Logging;
...
class Logger {
	...
	public static function log($level, $source, $message, array $context = array()) {
		...
		update_option(SNAP_CCB_CHURCH_CONNECT_LOG_OPTION, array_slice($entries, 0, self::MAX_ENTRIES), false);
	}

	public static function get_entries() {
		$entries = get_option(SNAP_CCB_CHURCH_CONNECT_LOG_OPTION, array());
		return is_array($entries) ? $entries : array();
	}

	public static function clear() {
		update_option(SNAP_CCB_CHURCH_CONNECT_LOG_OPTION, array(), false);
	}
