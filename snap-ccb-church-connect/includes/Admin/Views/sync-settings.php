...
$available_calendars = isset($options['available_calendars']) && is_array($options['available_calendars']) ? $options['available_calendars'] : array();
$selected_calendars = isset($options['selected_calendars']) && is_array($options['selected_calendars']) ? $options['selected_calendars'] : array();
?>
...
<form method="post" action="options.php">
	...
	<table class="form-table" role="presentation">
		...
		<tr><th scope="row"><label for="snap-ccb-selected-calendars"><?php echo esc_html__('Calendars to Sync', 'snap-ccb-church-connect'); ?></label></th><td><input type="hidden" name="<?php echo esc_attr(SNAP_CCB_CHURCH_CONNECT_OPTION); ?>[selected_calendars]" value=""><?php if ($available_calendars) : ?><select id="snap-ccb-selected-calendars" name="<?php echo esc_attr(SNAP_CCB_CHURCH_CONNECT_OPTION); ?>[selected_calendars][]" multiple size="<?php echo esc_attr((string) min(8, max(3, count($available_calendars)))); ?>"><?php foreach ($available_calendars as $calendar_key => $calendar_label) : ?><option value="<?php echo esc_attr($calendar_key); ?>" <?php selected(in_array((string) $calendar_key, $selected_calendars, true)); ?>><?php echo esc_html($calendar_label); ?></option><?php endforeach; ?></select><p class="description"><?php echo esc_html__('Leave empty to sync all discovered calendars. Hold Command/Ctrl to select more than one.', 'snap-ccb-church-connect'); ?></p><?php else : ?><p><?php echo esc_html__('No calendars have been discovered yet. Save credentials, then use Refresh Available Calendars below.', 'snap-ccb-church-connect'); ?></p><?php endif; ?></td></tr>
		...
		<tr><th scope="row"><?php echo esc_html__('Repeating Events', 'snap-ccb-church-connect'); ?></th><td><input type="hidden" name="<?php echo esc_attr(SNAP_CCB_CHURCH_CONNECT_OPTION); ?>[sync_non_repeating_only]" value="0"><label><input type="checkbox" name="<?php echo esc_attr(SNAP_CCB_CHURCH_CONNECT_OPTION); ?>[sync_non_repeating_only]" value="1" <?php checked($options['sync_non_repeating_only']); ?>> <?php echo esc_html__('Sync only non-repeating events', 'snap-ccb-church-connect'); ?></label><p class="description"><?php echo esc_html__('When enabled, sync checks CCB event_profile recurrence data and skips events that have a recurrence description. Existing repeating posts are not deleted automatically.', 'snap-ccb-church-connect'); ?></p></td></tr>
		<tr><th scope="row"><?php echo esc_html__('Multi-day Events', 'snap-ccb-church-connect'); ?></th><td><input type="hidden" name="<?php echo esc_attr(SNAP_CCB_CHURCH_CONNECT_OPTION); ?>[merge_multiday_occurrences]" value="0"><label><input type="checkbox" name="<?php echo esc_attr(SNAP_CCB_CHURCH_CONNECT_OPTION); ?>[merge_multiday_occurrences]" value="1" <?php checked($options['merge_multiday_occurrences']); ?>> <?php echo esc_html__('Merge consecutive CCB occurrences into one event', 'snap-ccb-church-connect'); ?></label><p class="description"><?php echo esc_html__('When CCB returns one occurrence per day for the same event, same time, and same location, sync will keep the first post, extend its end date, and delete the extra duplicate occurrence posts.', 'snap-ccb-church-connect'); ?></p></td></tr>
...
<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
	<?php wp_nonce_field('snap_ccb_church_connect_refresh_calendars'); ?>
	<input type="hidden" name="action" value="snap_ccb_church_connect_refresh_calendars">
	<?php submit_button(__('Refresh Available Calendars', 'snap-ccb-church-connect'), 'secondary', 'submit', false); ?>
</form>
<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
