<?php
namespace SnapChurchConnect\CCB\REST;
...
class EventsController {
	public function register_routes() {
		register_rest_route(
			SNAP_CCB_CHURCH_CONNECT_PUBLIC_REST_NAMESPACE,
			'/events',
			array(
	...
	public function get_event(\WP_REST_Request $request) {
		...
		if (! $post || 'church_event' !== $post->post_type || 'publish' !== $post->post_status) {
			return new \WP_Error('church_connect_event_not_found', __('Event not found.', 'snap-ccb-church-connect'), array('status' => 404));
		}
		if ('ccb' !== get_post_meta($post->ID, '_church_connect_provider', true)) {
			return new \WP_Error('church_connect_event_not_found', __('Event not found.', 'snap-ccb-church-connect'), array('status' => 404));
		}
